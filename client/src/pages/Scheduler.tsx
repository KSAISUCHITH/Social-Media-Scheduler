...
const handleSchedule = async (e: React.FormEvent) =>{
  ...
  return (<div className="flex flex-col lg:flex-row gap-6 h-full">
    ...
    <div className="w-full lg:w-[460px] shrink-0">
      <div className="bg-white rounded-2-xl border border-slate-200 p-6">
        ...
        <form className='space-y-5' onSubmit={handleSchedule}>
              ...
              <div className="flex flex-wrap gap-3">
                {PLATFORMS.map((p)=>{
                  ...
                  return(
                    ...
                    className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-red-50 border-red-300 text-red-500 scale-103" : 
            ...
            {mediaFile ? (
              <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                ...
                {mediaFile.type.startsWith("image/") ? <img src={URL.createObjectURL (mediaFile)} alt="preview"
                className="w-full h-40 object-cover"/> : <video src={URL.createObjectURL (mediaFile)} className="w-full h-40 object-cover" controls/>}
              ...
              <label className="flex items-center justify-center gap-2 p-5 py-10 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer
    ...
    <div className="flex-1 flex flex-col gap-6 min-w-0">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
          {scheduled.length === 0? (
            <div className="py-10 text-center text-slate-400 text-sm">No Posts Schedules yet</div> 
          ): (
            scheduled.map((post)=>(
              <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">
                      {post.mediaType}</span> }

                      <span className="text-xs text-slate-400"
                      >{new Date(post.scheduledFor).toLocaleString()}</span>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">
                      ...
                      <span className="text-xs text-slate-400"
                      >{new Date(post.updatedAt).toLocaleString()}</span>
