import { CalendarDaysIcon, LayoutDashboardIcon, UsersIcon, Wand2Icon,  LogOutIcon} from "lucide-react";
...
const Sidebar = ({isOpen,setIsOpen} : {isOpen: boolean, setIsOpen: (val:boolean)=> void} ) => {
  ...
  const{logout,user} = {
    ...
    <div
      className={
        `fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative 
        ...
        <nav className='flex-1 px-3 space-y-1'>
            {navItems.map((item)=>{
                ...
                return(
                    <NavLink key={item.name} to={item.path} end={item.path === "/dashboard"} onClick={()=> setIsOpen(false)}  
